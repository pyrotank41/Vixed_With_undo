...
//    the score, which should be restored from the list
//    the currentBoardIndex, which should be restored from the list
void addNodeToList(Board theBoard, int score, int currentBoardIndex, int moveNumber, Node* &pHead)
{
    // Create a new node and store current values into it
    Node* pTemp = new Node(theBoard, score, currentBoardIndex, moveNumber); // creating a new node.

    // Prepend it onto the front of the list
    pTemp->setNextPointer(pHead);
    pHead = pTemp;
}
...
int main()
{
    ...
    pHead = new Node(theBoard, score, currentBoardIndex, nodeMoveNumber);
